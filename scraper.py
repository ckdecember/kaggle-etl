...
class WebScrape:
    ...
    def readpage(self, page_request):
        ...
        soup = BeautifulSoup(
            page_request, features="html.parser", from_encoding=self.encoding
        )
        return soup
...
def main():
    ...
    ws = WebScrape()
    session = ws.get_url(
        "https://www.kaggle.com/account/login?phase=emailSignIn&returnUrl=%2Fwendykan%2Flending-club-loan-data%2Fversion%2F1"
    )
    print(session)
